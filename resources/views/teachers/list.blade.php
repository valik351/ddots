            ...
            @foreach($myTeachers as $teacher)
                <div class="col-lg-4 col-md-5">
                    <div class="card card-user">
                        <div class="content">
                            <div class="author">
                                <a href="{{ route('frontend::user::profile', ['id' => $teacher->id]) }}"><img class="avatar border-white" src="{{ $teacher->avatar }}" alt="..."></a>
                                <h4 class="title">
                            ...
                            <div class="text-center description">
                                @if(\Auth::check() && \Auth::user()->hasRole(App\User::ROLE_USER))
                                    <button type="button"
                                        ...
                                        My teacher</button>
                                    <button id="teacher_{{ $teacher->id }}" type="button" class="btn btn-success btn-fill btn-wd" style="display: {{ $teacher->relation_exists ? 'inline-block' : 'none' }}" disabled>request is sent</button>
                                @endif
                            </div>
                        </div>
                    </div>
